        ...
        <form method="POST" action="{{ route($submitRoute) }}" enctype="multipart/form-data" class="post-form">
            ...
            <div class="post-input">
                ...
                <div class="form-group">
                    <input type="text" id="caption" placeholder="Post Caption" class="@error('caption') is-invalid @enderror"
                ...
                <div class="form-group form-categories">
                    @foreach ($categories as $cat)
                        <span><input type="checkbox" class="@error('categories') is-invalid @enderror"
                                value="{{ $cat->id }}" id="categories" name="categories[]" @if (is_array(old('categories')) && in_array($cat->id, old('categories'))) checked @endif />
                            {{ $cat->name }}</span>
                    @endforeach
                </div>
